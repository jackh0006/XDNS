<p align="center">
  <img src="assets/xdns.png" alt="XDNS logo" width="160">
</p>
...
> [!NOTE]
> DNS tunneling با محدودیت اندازه payload، رفتار resolverها، latency، rate limit و packet loss روبه‌رو است. هدف XDNS اتصال قابل استفاده در شرایط سخت است، نه ادعای benchmark غیرواقعی یا جایگزینی VPN معمولی روی شبکه تمیز و پرسرعت.
...
| `internal/vpnproto` | ساخت، parse، payload و control packing packetهای XDNS |
...
```bash
./XDNS_Client_Linux_AMD64 --config client_config.toml
```
...
| 🧰 Service | نصب و start کردن service با نام `XDNS` |
...
```bash
bash <(curl -Ls https://raw.githubusercontent.com/WhiteDNS/XDNS/main/server_linux_install.sh) --version vYYYY.MM.DD.HHMMSS-abcdef0
...
```bash
./XDNS_Server_Linux_AMD64 --config server_config.toml
```
...
```bash
./XDNS_Server_Linux_AMD64 --config server_config.toml --udp-port 5353 --log-level DEBUG
```
...
```text
https://github.com/WhiteDNS/XDNS/releases/latest
```
...
| فایل | کاربرد |
| --- | --- |
| `XDNS_Client_*` | executable کلاینت |
...
```powershell
.\XDNS_Client_Windows_AMD64.exe --config client_config.toml
```
...
```bash
./XDNS_Client_Linux_AMD64 --config client_config.toml --listen-port 18001 --startup-mode logs
```
...
XDNS از فایل‌های TOML استفاده می‌کند. مسیرهای config از environment variable خوانده نمی‌شوند. مسیرهای runtime نسبت به executable/config location و با کمک `internal/runtimepath` و config helperها resolve می‌شوند.
...
کیفیت resolver تعیین می‌کند XDNS واقعاً قابل استفاده هست یا نه. ممکن است یک resolver برای DNS معمولی خوب باشد، اما برای label بزرگ تونل، queryهای تکراری، name طولانی یا rate limit خاص همان resolver مناسب نباشد. همیشه اجازه دهید کلاینت مسیر واقعی resolver/domain را تست کند.
...
sequenceDiagram
    ...
    participant Client as XDNS Client
    participant Resolver as DNS Resolver
    participant Server as XDNS Server
...
| 🔗 Chaining | یک proxy/panel محلی دیگر را به SOCKS5 listener XDNS chain کنید |
...
```text
XDNS_Client_<Platform>_<ARCH>.zip
XDNS_Client_<Platform>_<ARCH>.tar.gz
XDNS_Server_<Platform>_<ARCH>.zip
XDNS_Server_<Platform>_<ARCH>.tar.gz
```
...
XDNS بدون هیچ ضمانتی و به‌صورت as-is ارائه می‌شود. مسئولیت نحوه deploy، شبکه‌ای که روی آن استفاده می‌کنید و قانونی بودن استفاده در محل زندگی شما با خود شماست.
...
| 🧱 DNS server دیگری روی همان port اجرا نکنید | سرور XDNS به UDP/53 برای delegated tunnel traffic نیاز دارد |
...
| منبع | لینک |
| --- | --- |
| Issues | <https://github.com/WhiteDNS/XDNS/issues> |
| Pull requests | <https://github.com/WhiteDNS/XDNS/pulls> |
| Telegram | <https://t.me/whitedns> |

## 📄 مجوز

XDNS یک fork/derivative مستقل در خانواده MasterDnsVPN است. این repository
