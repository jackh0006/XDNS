#!/usr/bin/env bash
#
# XDNS Client Linux Installer
#
# Run this script from the directory where the release archive was extracted.
# It installs the XDNS client as a systemd service so it starts on boot.
...
echo -e "${MAGENTA}${BOLD}"
echo -e "          XDNS Client Auto-Installer${NC}"
echo -e "${CYAN}------------------------------------------------------${NC}"

require_cmd systemctl

stop_existing_XDNS_client_service() {
  if systemctl list-unit-files --all 2>/dev/null | grep -q '^XDNS-client\.service'; then
    log_info "Stopping existing XDNS client service..."
    systemctl stop XDNS-client 2>/dev/null || true

    for _ in 1 2 3 4 5; do
      if ! systemctl is-active --quiet XDNS-client; then
        break
      fi
      sleep 1
    done
    ...
    local main_pid
    main_pid="$(systemctl show XDNS-client --property MainPID --value 2>/dev/null || true)"
    if [[ -n "${main_pid:-}" && "$main_pid" != "0" ]] && kill -0 "$main_pid" 2>/dev/null; then
      log_warn "XDNS-client service is still active. Sending SIGTERM to MainPID: $main_pid"
      kill "$main_pid" 2>/dev/null || true
      sleep 2
      kill -0 "$main_pid" 2>/dev/null && kill -9 "$main_pid" 2>/dev/null || true
    fi

    systemctl reset-failed XDNS-client 2>/dev/null || true
  fi
}
...
log_header "Stopping Existing XDNS Client"
cleanup_legacy_cottenpickdns_client
stop_existing_XDNS_client_service

log_header "Locating Client Binary"
# The installer is bundled inside the release archive alongside the binary.
# Find the most recently modified XDNS_Client_* file in the current directory.
shopt -s nullglob
client_bins=(XDNS_Client_* XDNS_Client_*)
shopt -u nullglob
[[ ${#client_bins[@]} -eq 0 ]] && log_error "No XDNS_Client_* binary found in $INSTALL_DIR. Run this script from the extracted release directory."
...
SVC="/etc/systemd/system/XDNS-client.service"
cat > "$SVC" <<EOF
[Unit]
Description=XDNS Client
...
systemctl enable XDNS-client >/dev/null 2>&1 || log_warn "Could not enable XDNS-client service at boot."
systemctl restart XDNS-client

sleep 2
if ! systemctl is-active --quiet XDNS-client; then
  journalctl -u XDNS-client -n 50 --no-pager || true
  log_error "Service failed to start. See logs above."
fi

log_success "XDNS client service is running."

echo -e "\n${CYAN}======================================================${NC}"
echo -e " ${GREEN}${BOLD}       INSTALLATION COMPLETED SUCCESSFULLY!${NC}"
echo -e "${CYAN}======================================================${NC}"
echo -e "${BOLD}Commands:${NC}"
echo -e "  ${YELLOW}>${NC} Start:   systemctl start XDNS-client"
echo -e "  ${YELLOW}>${NC} Stop:    systemctl stop XDNS-client"
echo -e "  ${YELLOW}>${NC} Restart: systemctl restart XDNS-client"
echo -e "  ${YELLOW}>${NC} Logs:    journalctl -u XDNS-client -f"
echo -e "\n${BOLD}Files:${NC}"
...
echo -e "  ${YELLOW}>${NC} systemctl restart XDNS-client"
